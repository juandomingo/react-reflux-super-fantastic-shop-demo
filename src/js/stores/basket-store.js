  ...
  init() {
    ...
    onAddItem(item) {
      if (!item.inBasket) {
        ...
        let itemToIncrease = _.find(this.data.basketItems, { '_id': item._id, 'type': item.type });
    ...
    onRemoveItem(item){
      let itemToRemove = _.find(this.data.basketItems, { '_id': item._id, 'type': item.type });
    ...
    getBasketQty(item) {
      var basketItem = _.find(this.data.basketItems, { '_id': item._id, 'type': item.type });
